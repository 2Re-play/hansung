...
exports.message = async (req, res) => {
  ...
  try { 
    ...
    else if (content === '한성대입구역 -> 한성대 정문') response.expectBusRespondJson('🐢한성대입구역 -> 한성대 후문 방향 마을버스 도착예정 시간입니다.🐢\n', result.bus, result.buttons, res, 200)
    ...
    else if (content === '한성대 후문 -> 창신역,동묘역,동대문역') response.expectBusRespondJson('🐢한성대 후문 -> 창신역 방향 마을버스 도착예정 시간입니다.🐢\n', result.bus, result.buttons, res, 200)
    else if (content === '동묘역 -> 한성대 후문') response.expectBusRespondJson('🐢동묘역 -> 한성대 후문 방향 마을버스 도착예정 시간입니다.🐢\n', result.bus, result.buttons, res, 200)

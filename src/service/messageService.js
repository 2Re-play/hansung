...
exports.selectMessage = async (content, err) => {
  ...
  await new Promise(async (resolve, reject) => {
    ...
    else if (content === '한성대 후문 -> 창신역,동묘앞역,동대문역') result = await busInfo1(connection, content)
    else if (content === '동묘앞역 -> 한성대 후문') result = await busInfo1(connection, content)
    else if (content === '동대문역 -> 한성대 후문') result = await busInfo3(connection, content)
    else if (content === '한성대 미세먼지') result = await air(connection)
    else if (content === '면류&찌개&김밥') result = await menuList(content)
    else if (content === '덮밥류&비빔밥') result = await menuList(content)
    else if (content === '볶음밥&오므라이스&돈까스') result = await menuList(content)

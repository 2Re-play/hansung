...
exports.selectMessage = async (content, err) => {
  ...
  await new Promise(async (resolve, reject) => {
    ...
    else if (content === '창신역 -> 한성대 후문') result = await busInfo(connection, content)
    else if (content === '한성대 후문 -> 창신역') result = await busInfo(connection, content)
...
// 시즌
const busInfo = async (connection, content) => {
  try {
    const result1 = await commonButtons.busId(connection, content)
    const bus = await busData.busData(result1)
